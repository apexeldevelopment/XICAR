...
{{--                            <th>@lang("$string_file.vehicle_number")</th>--}}
                        ...
                        @foreach($drivers as $driver)
                            <tr>
                                <td><a href="{{ route('taxicompany.driver.show',$driver->id) }}"
                                ...
                                <td>
                                    @if($driver->total_trips)
                                        {{ $driver->total_trips }}  @lang("$string_file.ride")
                                    @else
                                ...
                                @if($config->driver_wallet_status == 1)
                                    <td>
                                        @if($driver->wallet_money)
                                            <a href="{{ route('taxicompany.driver.wallet.show',$driver->id) }}">{{ $driver->wallet_money }}</a>
                                        @else
                                            ------
                                        @endif
                                ...
                                <td>
                                    <div class="button-margin" >
                                        <a href="{{ route('taxicompany.driver.add',$driver->id) }}"
                                        ...
                                        @endif
                                        @if($driver->login_logout == 1)
                                            <a href="{{ route('taxicompany.driver.logout',$driver->id) }}"
    ...
    <div class="modal fade text-left" id="sendNotificationModel" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('taxicompany.sendsingle-driver') }}" method="post" enctype="multipart/form-data">
                    @csrf
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        ...
                        url: "{{route('taxicompany.drivers.delete')}}",
                    }).done(function (data) {
                        swal({
                            title: "DELETED!",
                            text: data,
                        ...
                        window.location.href = "{{ route('taxicompany.driver.index') }}";
                    });
                } else {
