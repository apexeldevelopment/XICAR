    ...
    <div class="page">
        <div class="page-content">
            ...
            <div class="panel panel-bordered">
                ...
                @php $tax_transfer_to = [1=> trans("$string_file.transfer_to_merchant") ,2=> trans("$string_file.transfer_to_store")]; @endphp
                ...
                <div class="panel-body container-fluid">
                    ...
                    {!! Form::hidden('id',old('id',isset($data['business_segment']['id']) ? $data['business_segment']['id'] : NULL),['id'=>'id']) !!}
...
{{--                                {!! Form::select('country_id',$data['countries'],old('country_id',isset($data['business_segment']['country_id']) ? $data['business_segment']['country_id'] : NULL),['id'=>'','class'=>'form-control','required'=>true]) !!}--}}
                        ...
                        @if(isset($tax_transfer_to_enable) && $tax_transfer_to_enable == 1)
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="firstName3">
                                        @lang("$string_file.tax_transfer_to")
                                    </label>
                                    {!! Form::select('tax_transfer_to',add_blank_option($tax_transfer_to,trans("$string_file.select")),old('tax_transfer_to',isset($data['business_segment']['tax_transfer_to']) ? $data['business_segment']['tax_transfer_to'] :NULL),['class'=>'form-control','required'=>true,'id'=>'tax_transfer_to']) !!}
                                    @if ($errors->has('tax'))
                        ...
                        @if($data['sub_group_for_app'] == 2)
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    {!! Form::select('grocery_configuration_instant_slot',$grocery_instant_slot,old('grocery_configuration_instant_slot',isset($data['business_segment']['grocery_configuration_instant_slot']) ? $data['business_segment']['grocery_configuration_instant_slot'] : NULL),['id'=>'grocery_configuration_instant_slot','class'=>'form-control','required'=>true]) !!}
                        ...
                        @if(in_array(6,$arr_merchant_service_type))
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    {!! Form::select('self_pickup',$data['self_pickup'],old('self_pickup',isset($data['business_segment']['self_pickup']) ? $data['business_segment']['self_pickup'] : NULL),['id'=>'self_pickup','class'=>'form-control','required'=>true]) !!}
                                </div>
                            </div>
                        @endif
                        @if(in_array(7,$arr_merchant_service_type))
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    {!! Form::select('dine_in',$data['dine_in'],old('dine_in',isset($data['business_segment']['dine_in']) ? $data['business_segment']['dine_in'] : NULL),['id'=>'dine_in','class'=>'form-control','required'=>true]) !!}
                                </div>
                            </div>
                        @endif
                        ...
                        @if(($data['slug'] == 'FOOD' || $data['slug'] == 'GROCERY') && $subscription_for_bs != 4)
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="emailAddress5">
                                    @lang("$string_file.order_based_on")
                                    <span class="text-danger">*</span>
                                </label>
                                {!! Form::select('order_based_on',$order_based_on,old('order_based_on',isset($data['business_segment']['order_based_on']) ? $data['business_segment']['order_based_on'] : NULL),['id'=>'order_based_on','class'=>'form-control','required'=>true]) !!}
                                @if ($errors->has('order_based_on'))
                                    <label class="text-danger">{{ $errors->first('order_based_on') }}</label>
                                @endif
                            </div>
                        </div>
                        @endif
                        @if($data['slug'] == 'FOOD')
                            ...
                            <div class="col-md-4 ">
                                <div class="form-group">
                                    ...
                                    {!! Form::number('minimum_amount',old('minimum_amount',isset( $data ['business_segment']['minimum_amount']) ? $data['business_segment']['minimum_amount'] : NULL),['id'=>'minimum_amount','class'=>'form-control','autocomplete'=>'off','min'=> 0]) !!}
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                {!! Form::text('address',old('address',isset($data['business_segment']['address']) ? $data['business_segment']['address'] : NULL),['id'=>'location','class'=>'form-control','required'=>true]) !!}
                                @if ($errors->has('address'))
                                    <label class="text-danger">{{ $errors->first('address') }}</label>
                                @endif
                            </div>
                        </div>
                        @if(isset($data['packaging_preference_in_store']) && $data['packaging_preference_in_store'])
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="emailAddress5">
                                        @lang("$string_file.packaging_preference")
                                        <span class="text-danger">*</span>
                                    </label>
                                    {!! Form::select('packaging_preference_enable',$data['packaging_preference_enable'],old('packaging_preference_enable',isset($data['business_segment']['packaging_preference_enable']) ? $data['business_segment']['packaging_preference_enable'] : NULL),['id'=>'packaging_preference','class'=>'form-control','required'=>true]) !!}
                                @if ($errors->has('packaging_preference'))
                                        <label class="text-danger">{{ $errors->first('address') }}</label>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="row">
                        @php
                            ...
                            $arr_slot_end_time = isset($data['business_segment']['slot_end_time']) ? json_decode($data['business_segment']['slot_end_time'],true) : NULL;
                        ...
                        @foreach($data['arr_day'] as $day_key=> $day)
                            ...
                            <div class="col-md-2">
                                ...
                                @if($bs_slot_end_time_enable == 1)
                                    <div class="mt-2">
                                            <label class="" style="margin-right: 10px;">
                                                <input type="radio" value="1" 
                                                    @if(!empty($arr_slot_end_time) && isset($arr_slot_end_time[$day_key]) && $arr_slot_end_time[$day_key] == 1) checked @endif 
                                                    name="slot_end_time[{{ $day_key }}]" onclick="calculate(this);"> @lang("$string_file.next_day")
                                            </label>
                                            
                                            <label class="">
                                                <input type="radio" value="2" 
                                                    @if(isset($arr_slot_end_time[$day_key]) && $arr_slot_end_time[$day_key] == 2) checked @endif 
                                                    name="slot_end_time[{{ $day_key }}]" onclick="calculate(this);"> @lang("$string_file.same_day")
                                            </label>
                                    </div>
                                @endif
                            </div>
                    ...
                    @if(isset($bank_details_admin_enable) && $bank_details_admin_enable == 1)
                    <hr>
...
@section('js')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            // Attach event listener for radio button selection
            $("#business-segment-form").on("submit", function () { 
                $("input[name^='slot_end_time']").each(function () {
                    var dayKey = $(this).attr('name').match(/\d+/)[0];
        
                    // Check if any radio button for this day is selected
                    var isChecked = $("input[name='slot_end_time[" + dayKey + "]']:checked").length > 0;
        
                    // If no radio button is selected, set "Same Day" (value 2) as default
                    if (!isChecked) {
                        $("input[name='slot_end_time[" + dayKey + "]'][value='2']").prop('checked', true);
                    }
                });
            });
            @if($bs_slot_end_time_enable == 1)
            // Attach event listener for timepicker field changes
            $("input[name^='close_time']").each(function () {
                $(this).on("change", function () {
                    handleTimeChange($(this));
                });
        
                // If using a timepicker plugin that fires events
                $(this).on("hide.timepicker", function () {
                    handleTimeChange($(this));
                });
            });
            
            // Attach event listener for timepicker field changes
            $("input[name^='open_time']").each(function () {
                $(this).on("change", function () {
                    handleOpenTimeChange($(this));
                });
            ...
            function handleOpenTimeChange(openTimeInput){
                var dayKey = openTimeInput.attr('name').match(/\d+/)[0]; // Extract day key
                var closeTimeInput = $("input[name='close_time[" + dayKey + "]']");
                
                var closeTime = closeTimeInput.val();
                var openTime = openTimeInput.val();
        
                // console.log(dayKey,closeTimeInput,closeTime,openTime);
                // Convert time strings to Date objects for comparison
                var startDateTime = new Date("1970-01-01T" + openTime + ":00");
                var endDateTime = new Date("1970-01-01T" + closeTime + ":00");
                console.log(endDateTime,startDateTime,endDateTime > startDateTime);
                // Check if close time is earlier than open time (indicating next day)
                if (endDateTime < startDateTime) {
                    alert("Close time is earlier than open time. Changing to 'Next Day'.");
                    $("input[name='slot_end_time[" + dayKey + "]'][value='1']").prop('checked', true);
                }else if(endDateTime > startDateTime){
                    alert("Changing to 'Same Day'.");
                    $("input[name='slot_end_time[" + dayKey + "]'][value='2']").prop('checked', true);
                }
               
            }

            function handleTimeChange(closeTimeInput) {
                var dayKey = closeTimeInput.attr('name').match(/\d+/)[0]; // Extract day key
                var openTimeInput = $("input[name='open_time[" + dayKey + "]']");
        ...
        function initialize() {
            ...
            var lat = "{{ isset($data['business_segment']['latitude']) ? $data['business_segment']['latitude'] : $default_lat }}";
            var long = "{{ isset($data['business_segment']['longitude']) ? $data['business_segment']['longitude'] : $default_long }}";
        ...
        function calculate(data) {
            var radio_q_value = $(data).attr('name').match(/\d+/)[0]; // Extract day_key
            var radio_type = $(data).val();
            console.log(radio_q_value,radio_type);
            
            // Identify input fields dynamically
            var current_start_time = $("input[name='open_time[" + radio_q_value + "]']").val();
            var current_end_time = $("input[name='close_time[" + radio_q_value + "]']").val();

            if (radio_type == 2) { // "Same Day" selected
                if (current_end_time < current_start_time) {
                    // alert("{{ trans($string_file.'.Not Possible to Change for Same Day as set for next day') }}");
                    alert('Not Possible to Change for Same Day as set for next day');
                     // Revert selection back to "Next Day"
                    $("input[name='slot_end_time[" + radio_q_value + "]'][value='1']").prop('checked', true);
                }
            }else{
                if (current_end_time > current_start_time) {
                    // alert("{{ trans($string_file.'.Not Possible to Change for Next Day as set for same day') }}");
                    alert('Not Possible to Change for Next Day as set for same day');
                     // Revert selection back to "Next Day"
                    $("input[name='slot_end_time[" + radio_q_value + "]'][value='2']").prop('checked', true);
                }
            }
        }
