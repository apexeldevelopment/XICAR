...
class UserController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->merge(['user_phone' => $request->phoneCode . $request->user_phone]);
        $request->validate([
            'first_name' => "required",
...
//        Onesignal::UserPushMessage($request->add_money_user_id, $data, $message, 3, $merchant_id);
        return redirect()->route('taxicompany.users.index')->withSuccess(trans("$string_file.money_added_successfully"));
    }
}