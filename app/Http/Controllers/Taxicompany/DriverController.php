...
class DriverController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $basicDriver = Driver::where([['taxi_company_id', '=', $taxicompany->id], ['signupStep', '<=', 4]])->latest()->count();
...
//        p($request->all());
    ...
    public function Wallet($id)
    {
        ...
        return view('taxicompany.driver.wallet', compact('wallet_transactions', 'driver'));
    }
    ...
    public function AllVehicle(Request $request)
    {
        ...
        $request->merge(['verification_status' => 'verified', 'for_taxi_company' => true]);
    ...
    public function PendingVehicle(Request $request)
    {
        ...
        $request->merge(['verification_status' => 'pending','for_taxi_company' => true]); // get pending vehicle
        $driver_vehicles = $this->getAllVehicles(true, $request);
        $areas = $this->getMerchantCountryArea($this->getAreaList(false,false,[],null,null, true)->get());
        $arr_search = $request->all();
        return view('taxicompany.drivervehicles.pending_vehicles', compact('driver_vehicles', 'areas', 'vehicles', 'arr_search'));
    }
    ...
    public function NewDriver()
    {
        $taxicompany = get_taxicompany();
        $merchant_id = $taxicompany->merchant_id;
        $drivers = Driver::where([['taxi_company_id', '=', $taxicompany->id], ['signupStep', '<=', 4]])->latest()->paginate(20);
        $areas = CountryArea::where([['merchant_id', '=', $merchant_id]])->latest();
...
//    }
    ...
    public function verifyDriver($id, $status)
    {
        ...
        try {
            ...
            return redirect()->route('taxicompany.driver.index')->withSuccess($msg);
        } catch (\Exception $e) {
    ...
    public function rejectDriver(Request $request)
    {
        ...
        //        p($request_from);
        if ($request_from == 1) {
            return redirect()->route('taxicompany.driver.rejected')->withSuccess($admin_panel_msg);
        } else {
            return redirect()->route('taxicompany.vehicle.rejected')->withSuccess($admin_panel_msg);
        }
    }
