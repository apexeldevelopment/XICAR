...
//    $string_file = $this->getStringFile($merchant_id);
    ...
    public function getPastBooking(Request $request)
    {
        ...
        $return_data = ['next_page_url' => $next_page_url, 'current_page' => $newArray['current_page'],'driver_total_earnings' => $driver->total_earnings, 'response_data' => $data];
...
//                            'location' => $booking->pickup_location,
            ...
            if (!empty($booking->BookingDetail)) {
                ...
                if (!empty($price)) {
                    $bill_details = HolderController::PriceDetailHolder($price, $request->booking_id, $currency, 'driver',NULL,'',$booking->merchant_id);
                    $grand_total = [
                        'name' => trans("$string_file.grand_total"),
                        'value' => $currency . $merchant_helper->PriceFormat($booking->final_amount_paid, $booking->merchant_id),
                        'colour' => "333333",
                        'bold' => true,
                    ];
            ...
            $dropAddress = isset($booking->BookingDetail) ? $booking->BookingDetail->end_location : '';
