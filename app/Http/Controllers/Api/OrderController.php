...
class OrderController extends Controller
{
        ...
        try {
            ...
            if ($order->order_status == 1  && !empty($order->id)) {
                ...
                } elseif ($request_status == "ACCEPT") // accepting order request
                {
                    ...
                    $order->Merchant->BookingConfiguration->multiple_delivery_ride_screen == 1 ? 2: 1;
                    $driver->save();
        ...
        try {
            ...
            foreach ($product_list as $product) {
                ...
                $bill_details[] = [
                    ...
                    'value' => $merchant_helper->PriceFormat(round_number($product->price + $option_amount), $order->merchant_id),
                    'bold' => false,
                ];
                //
            }
    ...
    function trackOrder(Request $request)
    {
        ...
        try {
            $order = Order::select("id", "driver_id", 'merchant_id', 'order_status', 'business_segment_id', 'poly_points', 'drop_latitude', 'drop_longitude')->Find($request->order_id);
            $string_file = $this->getStringFile($order->merchant_id);
            ...
            if($order->Merchant->ApplicationConfiguration->working_with_redis == 1){
                ...
                $driver_data = [
                    ...
                    'current_longitude' => isset($driver_coordinates['latitude']) ? $driver_coordinates['latitude'] : "",
            ...
            switch ($order_status) {
                case "6":
                    ...
                    $arr["movable_marker_type"] = [
                        ...
                        "driver_marker_lat"=> $driver_data['latitude'] ?? "",
                        "driver_marker_long"=> $driver_data['longitude'] ?? "",
    ...
    function trackOrderDetails(Request $request)
    {
        ...
        try {
            ...
            if($is_microservice_active && empty($order->User->user_jwt_token)){
                $user = $order->User;
                $jwt = getJwtToken($order->user_id, "multi-service-v3", "USER");
