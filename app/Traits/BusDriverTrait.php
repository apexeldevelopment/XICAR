...
trait BusDriverTrait
{
    // its getting list of merchant drivers
    public function getAllDriver($pagination = true, $request = NULL, $for_taxi_company = false, $for_driver_agency = false)
    {
        $permission_area_ids = [];
        ...
        // }
        $query = BusDriver::where([['merchant_id', '=', $merchant_id], ['driver_delete', '=', NULL]])
    ...
    // Bus Booking module
    public function driverByArea($area_id)
    {
        $bus = new BusDriver;
        $arr_buses = $bus->arrBuses($area_id);
        $return_buses = [];
        foreach ($arr_buses as $bus) {
            $return_buses[$bus->id] = $bus->vehicle_number . ' | ' . $bus->vehicle_color . ' | ' . $bus->VehicleType->VehicleTypeName;
        }
        return $return_buses;
    }
    public function arrDriverDropDown($arr_drivers,$string_file)
    {
        $drivers = "";
        if (empty($arr_buses)) {
            $buses = "<option value=''>" . trans("$string_file.no_service_area") . "</option>";
        } else {
            $drivers .= "<option value=''>" . trans("$string_file.select") . "</option>";
