    ...
    public function getOrderInfo($request)
    {
        $order = Order::with(['User' => function ($q) {
            ...
            ->with(['BusinessSegment' => function ($q) {
                $q->addSelect('id', 'full_name','country_area_id','phone_number', 'address', 'latitude', 'longitude');
            }])
            ->with(['OrderDetail' => function ($q) {
                $q->addSelect('id', 'order_id', 'price', 'weight_unit_id', 'product_id', 'quantity', 'product_variant_id', 'options','empty_bottle_quantity','empty_bottle_price','total_empty_bottle_price');
            }])
